...
instance Arbitrary Show1 where arbitrary = do ((e,o'),fs) <- runSuccess $ emptyExp $ arrExpOf' nullExp; p <- randPos; let o = mkObjC p [("result",o')] in return $ Show1 e o p fs

data Show2 = Show2 ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Show2 where arbitrary = do ((e,o'),fs) <- runSuccess $ arrExpOf' showableExp; p <- randPos; let o = mkObjC p [("result",o')] in return $ Show2 e o p fs

data Multi = Multi ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Multi where arbitrary = do ((e,o'),fs) <- runSuccess $ arrExpOf' atomExp; p <- randPos; let o = mkNumC p $ getMulti $ elems o' in return $ Multi e o p fs

data Mean = Mean ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Mean where arbitrary = do ((e,o'),fs) <- runSuccess $ arrExpOf' atomExp; p <- randPos; let o = mkNumC p $ getMean $ elems o' in return $ Mean e o p fs

data Desc = Desc ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Desc where arbitrary = do ((e,o'),fs) <- runSuccess $ arrExpOf' atomExp; p <- randPos; let o = mkTableC p (getDesc p $ elems o') [] in return $ Desc e o p fs
...
instance Arbitrary Table1 where
  arbitrary = do
    ...
    p <- randPos
    let o = mkTableC p (map elems $ elems o1) []
...
instance Arbitrary Table2 where
  arbitrary = do
    ...
    p <- randPos
    let o = mkTableC p (map elems $ elems o1) (elems $ flip fromMaybe (lookup "col" $ objElems o2) $ error "EnginePropSuccessUtils::arbitrary<Table2> [Couldn't lookup object key [col]]")
    return $ Table2 e1 e2 o p fs

data NTimes = NTimes ExpToken ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary NTimes where arbitrary = do ([(e1,o1),(e2,NumO _ n)],fs) <- runSuccess $ sequence [numExp, numExp]; p <- randPos; let o = mkArrC p $ replicate (floor n) o1 in return $ NTimes e1 e2 o p fs
...
instance Arbitrary Take1 where
  arbitrary = do
    ...
    p <- randPos
    let o = mkTableC p (map (take $ floor n) ess) h
    return $ Take1 e1 e2 o p fs

data Take2 = Take2 ExpToken ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Take2 where arbitrary = matrixArb Take2 $ \p es n -> mkArrC p $ take n es

data Sort1 = Sort1 ExpToken ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Sort1 where arbitrary = tableArb Sort1 $ \p ess h n -> mkTableC p (sortOn n ess) h

data Sort2 = Sort2 ExpToken ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Sort2 where arbitrary = matrixArb Sort2 $ \p es n -> mkArrC p $ sortArrOn n es

data Col1 = Col1 ExpToken ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Col1 where arbitrary = tableArb Col1 $ \p ess _ n -> mkArrC p $ ess !! n

data Col2 = Col2 ExpToken ExpToken ExpObj Pos [FuncEntryShow] deriving (Show)
instance Arbitrary Col2 where arbitrary = matrixArb Col2 $ \p es n -> let ArrO _ xs = es !! n in mkArrC p xs
...
instance Arbitrary Plott where
  arbitrary = do
    ...
    p <- randPos
    let o = mkPlotC p (zip ls rs) $ filter ((`elem` ["title","color"]) . fst) ps
...
objExpOfWithout' vs good = do
  ...
  p <- lift randPos
  expOrFunc' (mkObj' p $ zipWith mkPair ks ts) (mkObjC p $ zip ks os)
...
objExpOfWith' vs good = do
  ...
  p <- lift randPos
  expOrFunc' (mkObj' p $ zipWith mkPair keys ts) (mkObjC p $ zip keys os)
...
ignorePos (TableO _ ess h) = mkTableC p0 ess h
ignorePos (PlotO  _ ps  h) = mkPlotC  p0 ps  h
ignorePos (ArrO   _ es)    = mkArrC   p0 es
ignorePos (ObjO   _ ps)    = mkObjC   p0 ps
ignorePos (StrO   _ v)     = mkStrU   p0 v
ignorePos (NumO   _ v)     = mkNumU   p0 v
ignorePos (BoolO  _ v)     = mkBoolU  p0 v
ignorePos (NullO  _)       = mkNullU  p0
