...
instance Arbitrary SortIndexOutOfBounds2 where arbitrary = do width <- liftM ((+1).(`mod`10)) arbitrary; ((e2,_), fs0) <- runFailure $ arrExpOfLength' (arrExpOfLength' atomExp width) width; ((e1,o1),fs1) <- runStateT (chooseExp [numExpIn (-10) (-1),numExpIn width 1000]) fs0; return $ SortIndexOutOfBounds2 e1 e2 (getPos o1) 0 (width-1) (getIntVal o1) fs1
...
instance Arbitrary ColIndexOutOfBounds2 where arbitrary = do width <- liftM ((+1).(`mod`10)) arbitrary; ((e2,_), fs0) <- runFailure $ arrExpOfLength' (arrExpOfLength' atomExp width) width; ((e1,o1),fs1) <- runStateT (chooseExp [numExpIn (-10) (-1),numExpIn width 1000]) fs0; return $ ColIndexOutOfBounds2 e1 e2 (getPos o1) 0 (width-1) (getIntVal o1) fs1
