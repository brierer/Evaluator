...
import Data.Eval                        (ExpObj(..),FuncEntry(),Func(..))
...
fs = flip map funcs $ \(n,(typeValidators,_)) -> (n,(typeValidators, Func $ \_ -> success n))
...
addFunc fn r = let xs' = (fn,([],Func $ \_ -> return r)):fs; g = FuncT "" (IdT p0 ("","") fn) [] in (xs',g)
