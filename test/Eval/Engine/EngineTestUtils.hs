...
mk'  ts = mk p0 ts
mkO' ts = mkO p0 ts
mkObjFrom  ps = let (ss,tss) = unzip ps; ess = map (map un) tss in (concat ess,mkObj p0 $ zipWith (\x ys->mkPair p0 x $ mkArr p0 ys) ss ess)
mkObjFrom' ps = let (ss,ts)  = unzip ps; es  = map un ts        in (es,        mkObj p0 $ zipWith (mkPair p0)                        ss es)

oneArrayOf g1ras w1as = let es = map un g1ras; g1 = mkArr p0 es; (w1s,r) = mk' w1as in (funcMocks,g1,w1s,r)
...
toArray (x,y) e = mkArr p0 $ replicate ((x+y) `mod` 100) e
...
typeMismatchSortColCase name (P p) (NumTA _ g1) (TableValidArgs g2ss _) (TableOA g2r) (ExpTS w1) w2ass w2'as (ExpTS w2'') =
  let g2 = mkArr p0 $ map (mkArr p0) g2ss; w2ss = map (map un) w2ass 
      w2 = mkArr p0 $ map (mkArr p0) w2ss; (w2's,w2') = mk' w2'as; (xs,g2') = addFunc "table" g2r in 
...
emptySortColCase name pa pt n (TableValidArgs g2ss _) =
  let g2  = mkArr p0 $ map (mkArr p0) g2ss
      w2' = mkArr p0 $ map (mkArr p0) g2ss ++ [w2]
...
tableColumnLengthCase w1ps g2as = let ess = map snd w1ps in equalize ess /= ess ==>
  ...
  case applyFunc funcs p0 "table" [mkArr p0 $ map snd arrays, g2] of
...
tableHeaderLengthCase pa (TableValidArgs g1ss _) (TableValidArgs _ g2s) = 
  let g1 = mkArr p0 $ map (mkArr p0) g1ss
      w2 = mkObj p0 [mkPair p0 "col" $ mkArr pa g2s]
...
mkTableValidArgs pf g1ss g2s useHeader =
  let expectedHeader = concat [map unsafeMarshall g2s                       | useHeader]
      inputHeader    =        [mkPair p0 "col" $ mkArr p0 g2s | useHeader]
      g1 = mkArr p0 $ map (mkArr p0) g1ss
      g2 = mkObj p0 inputHeader
...
mkArrays a2ss = let arrays = map (mkArr p0) a2ss; aOfArrays = mkArr p0 arrays; mArrays = map unsafeMarshall arrays in (arrays,aOfArrays,mArrays)
