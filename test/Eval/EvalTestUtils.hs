...
import Eval.MultiPass                                 (State,Table,initState)
...
data ValidProg = ValidProg UniqueDefs deriving (Show)
...
instance Arbitrary ValidProg where
  ...
  shrink (ValidProg prog) = mValidProg (shrink prog) id
...
derefValidProg' = initState'.derefValidProg
initState' = (\(Right x)->x).initState.toToken
...
mValidProg  :: Monad m => m UniqueDefs -> ([String] -> m String) -> m ValidProg
mUndefProg  :: Monad m => m ValidProg  -> ([String] -> m String) -> m UndefProg
mCycleProg  :: Monad m => m ValidProg  -> ([String] -> m String) -> m CycleProg

derefValidProg  :: HasProg a => a -> a
derefValidProg' :: HasProg a => a -> State
fromProgForms   :: HasProg a => a -> Table
initState'      :: HasProg a => a -> State
