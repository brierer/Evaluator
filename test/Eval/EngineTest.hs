...
prop_TypeErrorTable (P p) g1ass (ObjTS g2) w1ass (ExpTS w1') (ExpTS w2) =
  let (_,g1) = mk g1ass; (w1ss,w1) = mk w1ass; in any (not.isArray) w1ss && not (isArray w1') && not (isObj w2) ==>
    arrayOf (array E.fs) w1  == applyFunc E.fs (mkFunc p "table" [w1 ,g2]) &&
    arrayOf (array E.fs) w1' == applyFunc E.fs (mkFunc p "table" [w1',w2]) &&
    obj E.fs w2              == applyFunc E.fs (mkFunc p "table" [g1 ,w2]) 
--    &&
...
prop_TypeErrorNTimes (P p) (NumTA _ g1) (NumTA _ g2) (ExpTS w1) (ExpTS w2) = not (isNum w1) && not (isNum w2) ==>
  num w1           == applyFunc E.fs (mkFunc p "nTimes" [w1,g2]) &&
  num w1           == applyFunc E.fs (mkFunc p "nTimes" [w1,w2]) &&
  num w2           == applyFunc E.fs (mkFunc p "nTimes" [g1,w2]) 
--  &&
...
prop_TypeErrorPlotLine (P p) g1as g2as (ObjTS g3) w1as (ExpTS w1') w2as (ExpTS w2') (ExpTS w3) =
  ...
  in any (not.isNum) w1s && not (isArray w1') && any (not.isNum) w2s && not (isArray w2') && not (isObj w3) ==>
    arrayOf num w1     == applyFunc E.fs (mkFunc p "plotLine" [w1 ,g2 ,g3]) &&
    arrayOf num w1     == applyFunc E.fs (mkFunc p "plotLine" [w1 ,w2 ,g3]) &&
    arrayOf num w1     == applyFunc E.fs (mkFunc p "plotLine" [w1 ,w2',g3]) &&
    arrayOf num w1     == applyFunc E.fs (mkFunc p "plotLine" [w1 ,g2 ,w3]) &&
    arrayOf num w1'    == applyFunc E.fs (mkFunc p "plotLine" [w1',g2 ,g3]) &&
    arrayOf num w1'    == applyFunc E.fs (mkFunc p "plotLine" [w1',w2 ,g3]) &&
    arrayOf num w1'    == applyFunc E.fs (mkFunc p "plotLine" [w1',w2',g3]) &&
    arrayOf num w1'    == applyFunc E.fs (mkFunc p "plotLine" [w1',g2 ,w3]) &&
    arrayOf num w2     == applyFunc E.fs (mkFunc p "plotLine" [g1 ,w2 ,g3]) &&
    arrayOf num w2     == applyFunc E.fs (mkFunc p "plotLine" [g1 ,w2 ,w3]) &&
    arrayOf num w2'    == applyFunc E.fs (mkFunc p "plotLine" [g1 ,w2',g3]) &&
    arrayOf num w2'    == applyFunc E.fs (mkFunc p "plotLine" [g1 ,w2',w3]) &&
    obj E.fs w3        == applyFunc E.fs (mkFunc p "plotLine" [g1 ,g2 ,w3]) 
--    &&
