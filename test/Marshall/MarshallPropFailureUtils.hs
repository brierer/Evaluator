...
data ArrOfLit = ArrOfLit [ExpToken] ExpObj Int Type deriving (Show)
...
data ObjOfLit = ObjOfLit [ExpToken] ExpObj Int Type deriving (Show)
...
data OrLit = OrLit [ExpToken] ExpObj Int Type deriving (Show)
...
data OrFunc = OrFunc [(ExpToken,ExpObj)] ExpObj Int Type deriving (Show)
...
mLit esa f isType mk = let onEmpty = mk [] (NullO p0) 0 in do
  es <- liftM (map un) esa
...
mOfLit asa chooseType chooseArg mk = let onEmpty = mk [] (NullO p0) 0 Null in do
...
mOfFunc tsa chooseType chooseArg mk = let onEmpty = mk [] (NullO p0) 0 Null in do
  ts <- liftM (map $ first clearParams . un) tsa
...
mOfLit  :: (Applicative m, Monad m, Unto b ExpToken) => m [b]          -> ([(Type,ExpToken  -> Bool)] -> m (Type,ExpToken -> Bool)) -> ([ ExpToken]         -> m ExpToken)          -> ([ExpToken]          -> ExpObj -> Int -> Type -> a) -> m a
mOfFunc :: (Applicative m, Monad m, Unto b ExpObj)   => m [(FuncTA,b)] -> ([(Type,ExpObj    -> Bool)] -> m (Type,ExpObj   -> Bool)) -> ([(ExpToken,ExpObj)] -> m (ExpToken,ExpObj)) -> ([(ExpToken,ExpObj)] -> ExpObj -> Int -> Type -> a) -> m a
