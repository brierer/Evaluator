...
test_ArgTypes = do   assertEqual (leafTypeMismatch (1,3) LeafTable LeafNull)  $ runFuncWith "f(null)"      $ singleTypeEntry "f" Table
                     assertEqual (leafTypeMismatch (1,3) LeafPlot  LeafTable) $ runFuncWith "f(mkTable())" $ singleTypeEntry "f" Plot ++ noArgFunc "mkTable" (\p -> TableO p [] [])
                     assertEqual (leafTypeMismatch (1,3) NodeArr   LeafPlot)  $ runFuncWith "f(mkPlot())"  $ singleTypeEntry "f" arr  ++ noArgFunc "mkPlot"  (\p -> PlotO  p [] [])
                     assertEqual (leafTypeMismatch (1,3) NodeObj   NodeArr)   $ runFuncWith "f([])"        $ singleTypeEntry "f" obj
                     assertEqual (leafTypeMismatch (1,3) LeafStr   NodeObj)   $ runFuncWith "f({})"        $ singleTypeEntry "f" Str
                     assertEqual (leafTypeMismatch (1,3) LeafNum   LeafStr)   $ runFuncWith "f(\"\")"      $ singleTypeEntry "f" Num
                     assertEqual (leafTypeMismatch (1,3) LeafBool  LeafNum)   $ runFuncWith "f(0)"         $ singleTypeEntry "f" Bool
                     assertEqual (leafTypeMismatch (1,3) LeafNull  LeafBool)  $ runFuncWith "f(false)"     $ singleTypeEntry "f" Null

                     assertEqual (leafTypeMismatch (1,13) LeafTable LeafPlot)  $ runFuncWith "f(mkTable(),mkPlot())" $ doubleTypeEntry "f" Table  ++ noArgFunc' "mkTable" (tableO [] []) ++ noArgFunc "mkPlot"  (\p -> PlotO p [] [])
                     assertEqual (leafTypeMismatch (1,12) LeafPlot  NodeArr)   $ runFuncWith "f(mkPlot(),[])"        $ doubleTypeEntry "f" Plot   ++ noArgFunc' "mkPlot"  (plotO  [] [])
                     assertEqual (leafTypeMismatch (1,6)  NodeArr   NodeObj)   $ runFuncWith "f([],{})"              $ doubleTypeEntry "f" arr
                     assertEqual (leafTypeMismatch (1,6)  NodeObj   LeafStr)   $ runFuncWith "f({},\"\")"            $ doubleTypeEntry "f" obj
                     assertEqual (leafTypeMismatch (1,6)  LeafStr   LeafNum)   $ runFuncWith "f(\"\",0)"             $ doubleTypeEntry "f" Str
                     assertEqual (leafTypeMismatch (1,5)  LeafNum   LeafBool)  $ runFuncWith "f(0,false)"            $ doubleTypeEntry "f" Num
                     assertEqual (leafTypeMismatch (1,9)  LeafBool  LeafNull)  $ runFuncWith "f(false,null)"         $ doubleTypeEntry "f" Bool
                     assertEqual (leafTypeMismatch (1,8)  LeafNull  LeafTable) $ runFuncWith "f(null,mkTable())"     $ doubleTypeEntry "f" Null  ++ noArgFunc "mkTable" (\p -> TableO p [] [])

                     assertEqual (leafTypeMismatch (1,10)  LeafBool                     LeafNull) $ runFuncWith "f([false,null])" $ singleTypeEntry "f" $ ArrOf Bool
                     assertEqual (leafTypeMismatch (1,10)  LeafNum                      LeafStr)  $ runFuncWith "f({x:0,y:\"\"})" $ singleTypeEntry "f" $ ObjOf Num
                     assertEqual (leafTypeMismatch (1,3)  (NodeOr [LeafBool, LeafNull]) LeafNum)  $ runFuncWith "f(0)"            $ singleTypeEntry "f" $ Or [Bool,Null]

                     assertEqual mockSuccess $ runFuncWith "f(mkTable())" $ ("f", [Table], mockSuccessFunc) : noArgFunc' "mkTable" (tableO [] [])
                     assertEqual mockSuccess $ runFuncWith "f(mkPlot())"  $ ("f", [Plot],  mockSuccessFunc) : noArgFunc' "mkPlot"  (plotO  [] [])
                     ...
                     assertEqual (leafTypeMismatch (1,3) NodeObj  NodeArr)   $ runFuncWith "f(mkArr())"  $ singleTypeEntry "f" obj  ++ noArgFunc "mkArr"  (`ArrO` [])
                     assertEqual (leafTypeMismatch (1,3) LeafStr  NodeObj)   $ runFuncWith "f(mkObj())"  $ singleTypeEntry "f" Str  ++ noArgFunc "mkObj"  (`ObjO` [])
                     assertEqual (leafTypeMismatch (1,3) LeafNum  LeafStr)   $ runFuncWith "f(mkStr())"  $ singleTypeEntry "f" Num  ++ noArgFunc "mkStr"  (`StrO` "")
                     assertEqual (leafTypeMismatch (1,3) LeafBool LeafNum)   $ runFuncWith "f(mkNum())"  $ singleTypeEntry "f" Bool ++ noArgFunc "mkNum"  (`NumO` 0)
                     ...
                     assertEqual (leafTypeMismatch (1,12) LeafPlot NodeArr)  $ runFuncWith "f(mkPlot(),mkArr())"   $ doubleTypeEntry "f" Plot ++ noArgFunc' "mkPlot" (mkPlotC  p0 [] []) ++ noArgFunc "mkArr"  (`ArrO` [])
                     ...
                     assertEqual (leafTypeMismatch (1,3)  (NodeOr [LeafBool, LeafNull]) LeafNum)  $ runFuncWith "f(mkNum())"               $ singleTypeEntry "f" (Or [Bool,Null]) ++ noArgFunc "mkNum"  (`NumO` 0)
